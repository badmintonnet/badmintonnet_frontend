...
export default function MemberScheduleDialog({
  ...
  const fetchSchedule = async () => {
    try {
      setLoading(true);
      const res = await clubServiceApi.getClubMemberSchedule(id, memberId);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-3xl max-h-[85vh] p-0 gap-0 rounded-2xl overflow-hidden bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-lg">
        <DialogHeader className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800 bg-gradient-to-r from-blue-50 to-white dark:from-gray-800/50 dark:to-gray-900/50">
          <div>
        ...
        <div className="px-6 py-4 overflow-y-auto max-h-[calc(85vh-120px)]">
          ...
          {!loading && items.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Thành viên này chưa tham gia sự kiện nào.
              </p>
            </div>
          )}
